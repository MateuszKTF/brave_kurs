...
**dnd-merchants-generator** — a Django 6.0 web app for D&D 5e Dungeon Masters to generate, persist, and price-adjust merchant shops. Single-user-per-data MVP, SQLite, deploy target Fly.io.

`@AGENTS.md` is the canonical onboarding doc (tripwires, structure, domain rules, scope guardrails). The most load-bearing points are repeated below so they aren't lost; read `@AGENTS.md` for the full picture and `@context/foundation/prd.md` (Polish) for authoritative requirements.

## Must know first

- **Use `py manage.py <cmd>` — not `python`, not `uv`.** Tech stack records `package_manager: uv`, but **uv is not installed and no virtualenv exists**. Django 6.0.6 was installed into global Python 3.14 via `py -m pip`.
- **Dependencies are unpinned** — no `requirements.txt` / `pyproject.toml` / lockfile. Create a real `.venv` before adding any dependency so you don't pollute the global interpreter.
- **Per-user data isolation is a hard requirement.** Every shop/merchant query MUST be scoped to `request.user`. A queryset that isn't owner-filtered is a bug. No admin/shared role in the MVP.
- **Python package is `dnd_merchants_generator` (underscores)**; product name is `dnd-merchants-generator` (hyphens). Settings module: `dnd_merchants_generator.settings`.
...
py manage.py test                                      # full suite
py manage.py test <app>.<TestClass>.<test_method>      # single test
```

## Architecture

The repo is bare `startproject` output — **no domain app exists yet**. `dnd_merchants_generator/` holds project config (settings, urls, wsgi/asgi); domain code goes in app packages you create with `startapp` and register in `INSTALLED_APPS`. DB is SQLite; auth uses Django's built-in `contrib.auth`.

Two business rules carry the product (see PRD Business Logic for authoritative detail):

1. **Assortment generation** — draw a random item set from a built-in SRD-derived catalog, filtered so item type and power/value match the shop's assortment type and town wealth. Randomness happens *within* a matched pool, not across the whole catalog (target ≥ 90% fit).
2. **Persuasion price recompute** — one Persuasion roll recomputes displayed prices of the *entire* item list against discount thresholds in < 1 s. **Base prices must be preserved** — this is a display transform, never a destructive write. Default thresholds (tunable): <10 → 0%; 10–14 → −5%; 15–19 → −10%; 20+ → −20%.

Persistence guarantee (`FR-008`): reopening a saved shop shows the exact same items and base prices as when saved.

## Conventions & guardrails

- **Currency is gold pieces only**, stored as integers — do not model copper/silver/electrum/platinum.
- `SECRET_KEY` is hardcoded and `DEBUG = True` (scaffold defaults) — move both to env config before any deploy.
- **Don't build** (PRD Non-Goals): sharing/public catalog · inventory/stock tracking · player-facing view · multi-currency · custom-artifact creator.
- `context/` is agent-context, not app code. **Never write to `context/archive/`** (immutable — open a new change instead).
