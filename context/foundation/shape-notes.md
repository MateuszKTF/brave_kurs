...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "rodzaj bólu"
      decision: "tarcie w przepływie pracy + dane uwięzione w notatkach + paraliż decyzyjny przy doborze przedmiotów"
    - topic: "persona scope"
      decision: "pojedynczy DM, w tym sam autor; tylko dla oczu DM-a"
    - topic: "auth strategy"
      decision: "e-mail + hasło; płaski model ról (sami DM-owie), izolacja danych per konto"
    - topic: "parametr doboru przedmiotów"
      decision: "poziom bogactwa miejscowości (np. wioska/miasteczko/metropolia) zamiast poziomu drużyny"
    - topic: "źródło bazy przedmiotów"
      decision: "import z gotowego źródła / SRD 5e, nie ręczne tworzenie"
    - topic: "priorytet regeneracji (FR-009)"
      decision: "awansowano z nice-to-have do must-have podczas rundy sokratejskiej"
    - topic: "ochrona przed usunięciem (FR-004)"
      decision: "dodano wymóg potwierdzenia przed nieodwracalnym usunięciem"
  frs_drafted: 10
  quality_check_status: accepted
---

# Shape Notes: DnD Merchants Generator

> Greenfield. Notatki źródłowe: `idea-notes.md`. Sekcje poniżej odwzorowują kolejność 10 sekcji PRD (greenfield).
...
- **Bez śledzenia stanu magazynowego** — „kupienie" przedmiotu przez gracza nie odejmuje go z asortymentu; brak wirtualnego koszyka i liczników sztuk. Upraszcza model danych i przepływ.
...
Kontrola jakości (Faza 7) nie wykazała luk — wszystkie wymagane elementy obecne:

- Access Control: present
- Business Logic (reguła w jednym zdaniu): present
- Project artifacts: present
- Timeline-cost acknowledged: present (mvp_weeks = 3 ≤ 3)
- Non-Goals: present

`quality_check_status: accepted`. Pozostają trzy Open Questions (patrz wyżej), żadna nie blokuje startu MVP.
